import os
import json
from tqdm import tqdm
import io


BLINK_ROOT = f'{os.path.abspath(os.path.dirname(__file__))}'

DATA_DIR = os.path.join(BLINK_ROOT, 'data', 'medmentions')
OUTPUT_DIR = os.path.join(BLINK_ROOT, 'data', 'medmentions', 'processed')

# get all of the documents
entity2idx = {}
documents = {}
src_documents = []
doc_dir = os.path.join(DATA_DIR, 'documents')
with open("./data/medmentions/documents/all_documents.json", 'r') as f:
    for idx, line in enumerate(f):
        one_doc = json.loads(line.strip())
        doc_id = one_doc['document_id']
        assert doc_id not in documents
        documents[doc_id] = one_doc
        entity2idx[doc_id] = len(src_documents)
        src_documents.append(one_doc)

splits = ['train', 'val', 'test']
mentions_documents = {}
for split in splits:
    with open("./data/medmentions/documents/" + split + ".json", 'r') as f:
        for idx, line in enumerate(f):
            one_doc = json.loads(line.strip())
            doc_id = one_doc['document_id']
            assert doc_id not in documents
            mentions_documents[doc_id] = one_doc


# get all of the train mentions
print('Processing mentions...')

splits = ['train', 'val', 'test']

for split in splits:
    print(f"Processing split: {split}")
    blink_mentions = []
    with open(os.path.join(DATA_DIR, 'mentions', split + '.json'), 'r') as f:
        for line in tqdm(f):
            one_mention = json.loads(line.strip())
            label_doc = documents[one_mention['label_document_id']]
            context_doc = mentions_documents[one_mention['context_document_id']]
            start_index = one_mention['start_index']
            end_index = one_mention['end_index']
            context_tokens = context_doc['text'].split()
            extracted_mention = ' '.join(context_tokens[start_index:end_index+1])
            assert extracted_mention == one_mention['text']
            context_left = ' '.join(context_tokens[:start_index])
            context_right = ' '.join(context_tokens[end_index+1:])
            transformed_mention = {}
            transformed_mention['mention'] = extracted_mention
            transformed_mention['mention_id'] = one_mention['mention_id']
            transformed_mention['context_left'] = context_left
            transformed_mention['context_right'] = context_right
            transformed_mention['context_doc_id'] = one_mention['context_document_id']
            transformed_mention['type'] = one_mention['category']
            transformed_mention['label_id'] = entity2idx[one_mention['label_document_id']]
            transformed_mention['label'] = label_doc['text']
            transformed_mention['label_title'] = label_doc['title']
            blink_mentions.append(transformed_mention)
    print('Done.')
    # write all of the transformed train mentions
    print(f'Writing {len(blink_mentions)} processed mentions to file...')
    with open(os.path.join(OUTPUT_DIR, split + '.jsonl'), 'w') as f:
        f.write('\n'.join([json.dumps(m) for m in blink_mentions]))
    print('Done.')

train_txt_file_path = "./data/medmentions/processed/train.jsonl"
test_txt_file_path = "./data/medmentions/processed/test.jsonl"
unseen_txt_file_path = "./data/medmentions/processed/test_unseen.jsonl"
seen_txt_file_path = "./data/medmentions/processed/test_seen.jsonl"
ids = set()

with io.open(train_txt_file_path, mode="r", encoding="utf-8") as file:
    for line in file:
        sample = json.loads(line.strip())
        ids.add(int(sample["label_id"]))

seen_mentions = []
unseen_mentions = []

with io.open(test_txt_file_path, mode="r", encoding="utf-8") as file:
    for line in file:
        sample = json.loads(line.strip())
        if int(sample["label_id"]) in ids:
            seen_mentions.append(sample)
        else:
            unseen_mentions.append(sample)

with open(unseen_txt_file_path, 'w') as f:
    f.write('\n'.join([json.dumps(m) for m in unseen_mentions]))
with open(seen_txt_file_path, 'w') as f:
    f.write('\n'.join([json.dumps(m) for m in seen_mentions]))
