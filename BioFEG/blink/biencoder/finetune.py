...
def main(params):
    ...
    # Load train data
    fname = os.path.join(params["generate_data_path"], "generate.t7")
    train_data = torch.load(fname)
    generate_feats = train_data["generate_feats"]
    candidate_input = train_data["candidate_vecs"]
    generate_feats = torch.tensor([item.cpu().detach().numpy() for item in generate_feats])
    candidate_input = torch.LongTensor([item.cpu().detach().numpy() for item in candidate_input])
    if params["zeshel"]:
        src_input = train_data['worlds'][:len(generate_feats)]
        train_tensor_data = TensorDataset(generate_feats, candidate_input, src_input)
    else:
        train_tensor_data = TensorDataset(generate_feats, candidate_input)
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            loss, _ = reranker(context_input, candidate_input, generate=True)
