...
def main(params):
    ...
    generate_feats = train_data["context_vecs"]
    candidate_input = train_data["candidate_vecs"]
    label_input = train_data["labels"]

    if params["zeshel"]:
        src_input = train_data['worlds'][:len(generate_feats)]
        train_tensor_data = TensorDataset(generate_feats, candidate_input, label_input, src_input)
    else:
        train_tensor_data = TensorDataset(generate_feats, candidate_input, label_input)
    ...
    fname = os.path.join(params["data_path"], "val.t7")
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            if params["zeshel"]:
                ...
                context_input, candidate_input, label_input = batch
            loss, _ = reranker(context_input, candidate_input, label_input, generate=True)
