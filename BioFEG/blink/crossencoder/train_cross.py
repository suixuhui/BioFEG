...
def main(params):
    ...
    fname = os.path.join(params["data_path"], "test_seen.t7")
    test_data = torch.load(fname)
    context_input = test_data["context_vecs"]
    candidate_input = test_data["candidate_vecs"]
    label_input = test_data["labels"]
    ...
    test_dataloader_seen = DataLoader(
        test_tensor_data,
        sampler=test_sampler,
        batch_size=params["eval_batch_size"]
    )

    fname = os.path.join(params["data_path"], "test_unseen.t7")
    test_data = torch.load(fname)
    context_input = test_data["context_vecs"]
    candidate_input = test_data["candidate_vecs"]
    label_input = test_data["labels"]
    ...
    test_dataloader_unseen = DataLoader(
        test_tensor_data,
        sampler=test_sampler,
        batch_size=params["eval_batch_size"]
    )
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            if epoch_idx == 0 or (epoch_idx == 1 and (step + 1) < 50000):
                continue
